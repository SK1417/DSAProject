...
class Bus {
    ...
    /////////////////////////////////////////////  Should add price member for each bus /////////////////////////////////////////////////
...
Bus search_for_route(Bus bus[], string route_num, int *no_of_buses) {
...
void return_subsequent_timings(string place, Bus bus[], int *no_of_buses, string route_num = "null") {
    time_t tt = time(0);
    tm* now = localtime(&tt);
    ...
    if(route_num != "null") {
        Bus specified_bus = search_for_route(bus, route_num, no_of_buses);
        int index_of_route = 0;
        for(int i=0; i<specified_bus.no_of_bus_stops; i++) 
            if(specified_bus.place_name[i] == place) {
                index_of_route = i+1;
                break;
            }
        int first_serv_time = specified_bus.first_serv_hour*60 + specified_bus.first_serv_min;
        int next_serv_time = curr_time - (curr_time-first_serv_time)%(60/specified_bus.freq) + (60/specified_bus.freq);
        cout<<"The next service for bus: "<<specified_bus.route_num<<" at "<<place<<" would be at "<<(next_serv_time/60)<<":"<<(next_serv_time%60);
    }
    else {
        for(int i=0; i<*no_of_buses; i++) {
...
void list_all_routes(Bus bus[], Route route[], int *no_of_buses) {
    ...
    else
    {
        ...
        ///////////////////////// Have to add price details here ////////////////////////////////
