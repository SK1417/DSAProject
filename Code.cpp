...
int find(Linked_place *head , string str){
	if(head->data == str)
		return 1;
	else
		find(head->next , str);
...
void insert(Linked_place *head , Bus bus[] , int *no_of_buses) {
	for(int i = 0 ; i < (*no_of_buses) ; i++) {
		for(int j = 0 ; j < bus[i].no_of_bus_stops ; j++) {
			if(find(Linked_place *head , bus[i].place_name)) 
				return;
			else {
				if(head == NULL) {
					Linked_place *ptr = new Linked_place;
					head = ptr;
					ptr -> data = bus[i].place_name;
					ptr->next = NULL;
				}
				else {
					Linked_place *L = new Linked_place;
					Linked_place *ptr;
					ptr = head;
					while(ptr->next!=NULL)
						ptr = ptr->next;
					ptr -> next = L;
					L -> data = bus[i].place_name;
					L -> next = NULL;
				}
			}
		}
